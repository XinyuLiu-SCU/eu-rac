"""
ChicagoEnv ,SiouxEnv-compatible environment for the Chicago Sketch network.

Same interface as SiouxEnv but loads Chicago Sketch network data.
"""
import numpy as np
import pandas as pd
...
class ChicagoEnv:
    ...
    def _load_network(self):
        net_df = pd.read_csv(NETWORK_DIR / 'Chicago_Sketch_network.csv')
        sigma_arr = np.load(NETWORK_DIR / 'Chicago_Sketch_0.4_random_sigma.npy')
    ...
    def _setup_explicit_uncertainty(self, edges_dict):
        ...
        for node, spec in edges_dict.items():
            ...
            if isinstance(spec, dict):
                ...
                if spec.get('executed') is not None:
                    executed = int(spec['executed'])
                    if executed == intended or executed not in self.successors.get(node, []):
                        raise ValueError(f"uncertain_edges_explicit: invalid executed edge {node}->{executed}")
                    entry['executed'] = executed
                self.uncertain_edges[node] = entry
            else:
                next_node = int(spec)
    ...
    def _let_uncertainty_exposure(self, path):
        ...
        risky = sum(
            ...
            if (self.uncertain_edges.get(u, {}).get('intended') if isinstance(self.uncertain_edges.get(u), dict) else self.uncertain_edges.get(u)) == v
        )
    ...
    def sample_executed_action(self, node, intended_action, p_intended=None):
        """Execute intended action unless a configured uncertainty event occurs."""
        prob = p_intended if p_intended is not None else self.exec_prob
        if node not in self.uncertain_edges:
            return intended_action
        spec = self.uncertain_edges[node]
        if isinstance(spec, dict):
            if intended_action != spec['intended']:
                return intended_action
            if self.rng.random() < prob:
                return intended_action
            if spec.get('executed') is not None:
                return int(spec['executed'])
            alternatives = [n for n in self.successors[node] if n != intended_action]
            return int(self.rng.choice(alternatives)) if alternatives else intended_action
        if intended_action != spec or self.rng.random() < prob:
            return intended_action
        alternatives = [n for n in self.successors[node] if n != intended_action]
        return int(self.rng.choice(alternatives)) if alternatives else intended_action
