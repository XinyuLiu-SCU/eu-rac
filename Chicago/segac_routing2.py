...
_NET_DIR = Path(__file__).parent.parent / 'Networks' / 'Networks' / 'Chicago_Sketch'
_net_df = __import__('pandas').read_csv(_NET_DIR / 'Chicago_Sketch_network.csv')
_nodes_set = set()
...
def _precompute_let_to_go(dest):
    ...
    for _, row in _net_df.iterrows():
        ...
        reverse.setdefault(v, []).append((u, max(1, round(float(row['Cost'])))))
    dist = {dest: 0}
    pq = [(0, dest)]
    while pq:
