"""
BA_mean.py - Barcelona network batch OD x eta comparison experiment.

Usage:
    python BA_mean.py --run-all          # run all OD pairs from --od-source
    python BA_mean.py --od 663 701       # run specified OD only
    python BA_mean.py --aggregate-only   # re-compute mean from existing CSVs
"""
# Algorithms: EU-RAC, GE-DDRL, and DAC.

DEBUG_OD_CONFIGS = {
    (213, 295):  {},   # t_LET=4.32h,  path_len=12, min_b=12, score=3.23 (rank 1)
    (389, 295):  {},   # t_LET=4.16h,  path_len=14, min_b=14, score=3.21 (rank 2)
    (386, 245):  {},   # t_LET=6.23h,  path_len=15, min_b=15, score=3.14 (rank 3)
    (84, 374):   {},   # t_LET=5.22h,  path_len=12, min_b=12, score=3.11 (rank 4)
    (297, 245):  {},   # t_LET=5.94h,  path_len=12, min_b=12, score=2.90 (rank 5)
}

ETA_LIST = [0, 0.2, 0.4]          
B_COEFFS = [0.975, 1, 1.025]       # Chicago protocol  minute-scale ensures budget separation


GLOBAL_UNCERTAIN_RATIO = 0.2
# Barcelona: seed=15 provides meaningful  impact across 5 ODs
# (same methodology as Sioux/Anaheim/Chicago).
GLOBAL_UNCERTAIN_SEED = 15

# OD-conditioned calibration disabled for Barcelona by default.
# Can be re-enabled after calibrating target/max ratios for Barcelona ODs.
USE_OD_UNCERTAINTY = False
TARGET_UNCERTAIN_RATIO = 0.15
MAX_UNCERTAIN_RATIO = 0.20
OD_UNCERTAIN_SEED = 20260710
...
from ge_ddrl_routing3 import run_ge_ddrl_routing  # Barcelona-adapted GE-DDRL v3
...
_net_df = pd.read_csv(_NET_DIR / 'Barcelona_network.csv')

_mean_edges = {}
_successors = {}
for _, row in _net_df.iterrows():
    u, v = int(row['From']), int(row['To'])
    _mean_edges[(u, v)] = float(row['Cost']) * 60   # hours  minutes
...
CHECKPOINT_INTERVAL = 10000  # save checkpoint every N episodes
DEFAULT_OD_SOURCE = OUTPUT_DIR / 'selection' / 'barcelona_final20.csv'
...
def _checkpoint_path(origin, dest, budget_coeff, eta, algorithm='eurac'):
    """Build checkpoint path: checkpoints/OD_O_D/B_XXX/eta_X/alg.pt"""
    ckpt_dir = CHECKPOINT_DIR / f'OD_{origin}_{dest}' / f'B_{budget_coeff}' / f'eta_{eta}'
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    return ckpt_dir / f'{algorithm}.pt'


def load_uncertainty_environments(path=DEFAULT_UNCERTAINTY_FILE):
    path = Path(path)
    if not path.exists():
        print(f'[BA_mean] uncertainty file not found: {path}')
        return {}
    ...
    for key, info in raw.items():
        ...
        if edge_specs:
            explicit = {
                int(edge['u']): {'intended': int(edge.get('v', edge.get('intended')))}
...
def load_final_od_pairs(path=DEFAULT_OD_SOURCE):
    path = Path(path)
    if not path.exists():
        print(f'[BA_mean] OD source not found: {path}; falling back to uncertainty JSON order')
    ...
    # Handle both 'dest' (selection CSV) and 'destination' (legacy CSV) column names
    dest_col = 'dest' if 'dest' in df.columns else 'destination'
    return [(int(row['origin']), int(row[dest_col])) for _, row in df.iterrows()]
...
def run_all_methods(env, origin, dest, budget, combo_seed, selected_methods=None, uncertain_edges_explicit=None,
                    eurac_ckpt_path=None, force=False):
    ...
    def _make_mc_env():
        ...
        return BarcelonaEnv(
            ...
            use_od_uncertainty=False if uncertain_edges_explicit is not None else USE_OD_UNCERTAINTY,
            od_target_uncertain_ratio=TARGET_UNCERTAIN_RATIO,
            od_max_uncertain_ratio=MAX_UNCERTAIN_RATIO,
            od_uncertain_seed=OD_UNCERTAIN_SEED,
        )
    ...
    if _run_method('ILP'):
        ...
        print(f'  ILP: algo={_t_algo:.1f}s')

    if _run_method('OTAP'):
        # ---- OTAP (K=30, heuristic) ----
        _t0 = time.time()
        r = run_otap_routing(env, budget=budget, K=30)
        _t_algo = time.time() - _t0
        path = r['path']
        pm = {path[i]: path[i + 1] for i in range(len(path) - 1)}
        results['OTAP'] = _mc_eval(lambda n, b: pm.get(n), 'OTAP')
        print(f'  OTAP: algo={_t_algo:.1f}s')
    ...
    if _run_method('GE-DDRL'):
        # ---- GE-DDRL (Barcelona-adapted tabular C51 v3) ----
        _t0 = time.time()
        print(f'    GE-DDRL: Barcelona-adapted per-edge tabular Z')
        print(f'    Warm-start target: LET-to-go')
        r = run_ge_ddrl_routing(env, budget=budget, N=800, delta_w=1.0,
    ...
    if _run_method('EU-RAC'):
        ...
        _start_ep = 1
        _ckpt_path = eurac_ckpt_path
        _loaded_from_ckpt = False

        #  Resume from checkpoint if available 
        if _ckpt_path is not None and _ckpt_path.exists() and not force:
            try:
                _ckpt_state = agent.load_checkpoint(_ckpt_path)
                _start_ep = _ckpt_state.get('episode', 1) + 1
                _loaded_from_ckpt = True
                print(f'    [EU-RAC] Resumed from checkpoint: episode {_start_ep}/{EURAC_EPISODES}')
                print(f'    [EU-RAC] Checkpoint saved at: {_ckpt_path}')
            except Exception as e:
                print(f'    [EU-RAC] WARNING: checkpoint load failed ({e}), starting from scratch')

        #  Warm-start (skip if checkpoint loaded) 
        if not _loaded_from_ckpt:
            ...
            _agree_before = agent.compute_dijkstra_agreement()
            agent._diag_dijkstra_agreement_before = _agree_before
            print(f'    [EU-RAC] Dijkstra agreement (after resume): {_agree_before:.3f}')
        ...
        for ep in range(_start_ep, EURAC_EPISODES + 1):
            agent.run_episode(train=True)
            #  Periodic checkpoint save 
            if _ckpt_path is not None and ep % CHECKPOINT_INTERVAL == 0:
                agent.save_checkpoint(_ckpt_path, episode=ep)
            if ep % _progress_interval == 0:
                _elapsed = time.time() - _t0
                _eps_per_s = ep / _elapsed if _elapsed > 0 else 0
                print(f'    EU-RAC progress: {ep}/{EURAC_EPISODES} episodes, '
                      f'{_elapsed:.0f}s elapsed, {_eps_per_s:.0f} eps/s')

        #  Save final checkpoint 
        if _ckpt_path is not None:
            agent.save_checkpoint(_ckpt_path, episode=EURAC_EPISODES)
            print(f'    [EU-RAC] Final checkpoint saved: {_ckpt_path}')

        _t_algo = time.time() - _t0
        qe_avg, qd_avg = agent.q_stats()
        _avg_eps_time = _t_algo / EURAC_EPISODES * 1000  # ms per episode
        ...
        def eurac_policy(node, bud):
            ...
            ranked = sorted(valid_acts, key=lambda a: pol.get(a, 0.0), reverse=True)
            original_argmax = ranked[0]
            selected = None
...
def run_one_od(origin, dest, force=False, selected_methods=None, uncertainty_envs=None):
    ...
    print()
    print(f'{"=" * 60}')
    print(f'  OD {origin}->{dest}  |  t_LET = {t_let:.1f} min  |  min budget = {min_budget:.0f}')
    if explicit_uncertainty is not None:
        ...
        if USE_OD_UNCERTAINTY:
            print(
                f'  OD calibration: target={TARGET_UNCERTAIN_RATIO:.0%}, '
                f'max={MAX_UNCERTAIN_RATIO:.0%}, seed={OD_UNCERTAIN_SEED}'
            )
    print(f'{"=" * 60}')
    ...
    for coeff, budget, eta, key in all_tasks:
        ...
        env = BarcelonaEnv(
            ...
            use_od_uncertainty=False if explicit_uncertainty is not None else USE_OD_UNCERTAINTY,
            od_target_uncertain_ratio=TARGET_UNCERTAIN_RATIO,
            od_max_uncertain_ratio=MAX_UNCERTAIN_RATIO,
            od_uncertain_seed=OD_UNCERTAIN_SEED,
        )
        ...
        t0 = time.time()
        eurac_ckpt = _checkpoint_path(origin, dest, coeff, eta, 'eurac')
        mc_vals = run_all_methods(env, origin, dest, budget, combo_seed, selected_methods=selected_methods, uncertain_edges_explicit=explicit_uncertainty,
                                  eurac_ckpt_path=eurac_ckpt, force=force)
...
# ============================================================
def parse_args():
    p = argparse.ArgumentParser(description='Barcelona Batch OD x eta experiments')
...
def main():
    ...
    print(f'[BA_mean] Method implementations:')
    ...
    print(f'  GE-DDRL:  Barcelona-adapted tabular C51 v3 - ge_ddrl_routing3.py')
    ...
    if not od_pairs:
        print(f'[BA_mean] no OD pairs found in {args.od_source}')
        return
