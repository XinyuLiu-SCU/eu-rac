...
# Network dimensions are derived from the runtime environment.
_NUM_NODES = 1


def _precompute_let_to_go(dest):
    """Compatibility helper for local checks; not used by SEGAC training."""
    raise RuntimeError("Use runtime env edges; this compatibility helper is not used by SEGAC training.")
    reverse = {}
    for _, row in _net_df.iterrows():
        u, v = int(row['From']), int(row['To'])
        reverse.setdefault(v, []).append((u, max(1, round(float(row['Cost'])))))  # hours  minutes
    dist = {dest: 0}
    pq = [(0, dest)]
    while pq:
...
def run_segac_routing(env, budget: int,
    ...
    def segac_neural_policy(node, bud):
        return _policy_action(node, bud, None)
