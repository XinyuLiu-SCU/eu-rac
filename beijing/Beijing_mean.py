"""Official Beijing benchmark runner and Phase 3 smoke-test entry point."""
...
PUBLIC_OD_FILE = ROOT.parent / "data" / "beijing" / "od_pairs.csv"

PERIOD_DATA_FILES = {
    "weekday_peak": NETWORK_DIR / "Beijing_Weekday_peak.csv",
    "weekday_offpeak": NETWORK_DIR / "Beijing_Weekday_Offpeak.csv",
    "weekend_peak": NETWORK_DIR / "Beijing_Weekend_Peak.csv",
    "weekend_offpeak": NETWORK_DIR / "Beijing_Weekend_Offpeak.csv",
}

PROTECTED_FILES = [
    NETWORK_DIR / "Beijing_Network.xlsx",
    NETWORK_DIR / "Beijing_Pairs.npy",
    PUBLIC_OD_FILE,
    PERIOD_DATA_FILES["weekday_peak"],
    PERIOD_DATA_FILES["weekday_offpeak"],
    PERIOD_DATA_FILES["weekend_peak"],
    PERIOD_DATA_FILES["weekend_offpeak"],
]

CANONICAL_ALGORITHMS = [
    "dot", "robust", "pulse", "gp3", "ilp", "otap", "pql", "segac", "dac", "geddrl", "eurac"
]
...
def make_env(period: str, origin: int, dest: int, budget: int, eta: float, seed: int | None,
             uncertain_edges_explicit: dict | None = None) -> BeijingEnv:
    if eta == 0 and uncertain_edges_explicit is None:
        return BeijingEnv(origin=origin, destination=dest, budget=budget, eta=0.0, period=period, seed=seed)
...
def checkpoint_path(period: str, alg: str, origin: int, dest: int, ratio: float, eta: float) -> Path:
    return CHECKPOINT_DIR / period / alg / f"OD_{origin}_{dest}" / f"B_{ratio:g}" / f"eta_{eta:g}"
...
def validate_path(env: BeijingEnv, path: Any) -> tuple[bool, bool, str]:
...
def extract_result_path(result: dict[str, Any], env: BeijingEnv, budget: int) -> list[int] | None:
...
def rollout_greedy_path(env: BeijingEnv, policy: Callable[[int, int], int | None], budget: int) -> list[int]:
    node = int(env.origin)
    dest = int(env.dest)
...
def run_algorithm(alg: str, env: BeijingEnv, period: str, origin: int, dest: int, budget: int,
    ...
    ckpt = checkpoint_path(period, alg, origin, dest, ratio, eta)
    ...
    try:
        ...
        if alg == "dot":
            ...
            os.environ["BEIJING_PERIOD"] = period
            ...
            def factory(eval_seed: int) -> BeijingEnv:
            ...
            agent.train(n_episodes=episodes, checkpoint_episodes=checkpoint_episodes, checkpoint_dir=ckpt, env_factory=factory, val_episodes=mc_runs, val_seed=seed + 9010)
...
def write_results_csv(rows: list[dict[str, Any]]) -> Path:
    path = REPORT_DIR / "Beijing_Phase3_Smoke_Test_Results.csv"
    path.parent.mkdir(parents=True, exist_ok=True)
    fields = [
...
def legacy_audit() -> list[tuple[str, str, str]]:
    files = [ROOT / "Beijing_mean.py", ROOT / "eu_rac.py", ROOT / "dac_routing2.py", ROOT / "segac_routing2.py", ROOT / "ge_ddrl_routing3.py"]
...
def max_high_budget() -> int:
    ...
    for period in PERIOD_DATA_FILES:
        ...
        env0 = BeijingEnv(period=period, eta=0.0)
...
def write_reports(rows: list[dict[str, Any]], before_hashes: dict[str, str], after_hashes: dict[str, str],
    ...
    runner_path = REPORT_DIR / "Beijing_Phase3_Runner_Report.md"
    smoke_path = REPORT_DIR / "Beijing_Phase3_Smoke_Test_Report.md"
    ...
    runner = f"""# Beijing Phase 3 Runner Report

## Files Modified

- beijing/Beijing_mean.py: repaired in place as the official Beijing runner.
- beijing/eu_rac.py: repaired import-time network loading to use BEIJING_PERIOD and raw seconds.
- beijing/dac_routing2.py: repaired import-time network loading to use BEIJING_PERIOD and raw seconds.

## Backup Files

- beijing/Beijing_mean.py.bak_before_phase3
- beijing/eu_rac.py.bak_before_phase3
- beijing/dac_routing2.py.bak_before_phase3
...
The selected period is normalized through BeijingEnv.normalize_period, mapped to the matching period-specific Release OD NPY, and passed into every BeijingEnv instance.
...
Outputs are written under beijing/outputs/<period>/.
Checkpoints are written under beijing/checkpoints/<period>/<algorithm>/OD_<origin>_<destination>/B_<ratio>/eta_<eta>/.
Smoke-test reports are written under beijing/report and beijing/report/smoke_tests/<period>/.
...
The maximum Beijing Release OD high budget observed during this audit is {max_b} seconds.
...
"""
    ...
    smoke = f"""# Beijing Phase 3 Smoke Test Report
...
def load_uncertain_actions(period: str, uncertainty_file: Path | None = None) -> dict | None:
    ...
    for item in actions:
        ...
        mapping[node] = {'intended': intended}
        if 'alternative' in item:
            mapping[node]['alternative'] = int(item['alternative'])
        if 'deviation_successors' in item:
            mapping[node]['deviation_successors'] = [int(v) for v in item['deviation_successors']]
        if 'failure_delay' in item:
            mapping[node]['failure_delay'] = float(item['failure_delay'])
    return mapping
                ...
                for eta in etas:
                    for alg in algorithms:
                        for run_index in range(1, args.repeats + 1):
                            seed = int(args.seed + run_index + origin * 101 + dest * 503 + int(ratio * 1000) * 1009 + int(eta * 1000) * 9173)
                            tasks.append({
                                'period': period,
                                'od_index': od_index,
                                'origin': origin,
...
def aggregate_results(results_dir: Path = RESULTS_DIR) -> Path:
    ...
    for path in sorted(results_dir.glob('*/*/B_*/OD_*/run_*.json')):
        try:
            ...
            continue
        rows.append(data)
    out = REPORT_DIR / 'Beijing_Benchmark_Aggregate.csv'
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Official Beijing benchmark runner")
    parser.add_argument("--period", default="all", choices=list(PERIOD_DATA_FILES) + ["all"], help="Beijing period, or all for batch mode")
    ...
    parser.add_argument("--od-index", type=int, default=-1, help="Release OD index; -1 runs every Release OD OD in batch mode")
    ...
    if args.smoke_test:
        ...
        algorithms = parse_algorithms(args.algorithms)
        validation_commands = ["python Beijing_mean.py --help"]
        validation_commands.append("python Beijing_mean.py --period weekday_peak --algorithms dot --smoke-test")
        validation_commands.append("python Beijing_mean.py --period weekday_peak --algorithms dot robust pulse gp3 ilp otap pql segac dac geddrl eurac --smoke-test")
        validation_commands.append("python Beijing_mean.py --period weekday_offpeak --algorithms pulse --od-index 0 --budget-ratio 1.0")
        validation_commands.append("python Beijing_mean.py --period weekend_peak --algorithms pulse --od-index 0 --budget-ratio 1.0")
        validation_commands.append("python Beijing_mean.py --period weekend_offpeak --algorithms pulse --od-index 0 --budget-ratio 1.0")
        rows = run_smoke(args)
