...
for _, row in _net_df.iterrows():
    ...
    _mean_edges[(u, v)] = float(row['Cost']) / 0.25
...
def run_dac_routing(env, budget: int,
    ...
    for ep in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            t = _sample_tt(node, action); b_next = b - max(1, round(t))
    ...
    for _ in range(max_steps):
        ...
        mean_t, _ = edges[(node, action)]; b -= max(1, round(mean_t))
