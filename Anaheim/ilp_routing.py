...
def run_ilp_routing(env, budget: int, K: int = 200) -> dict:
    ...
    runtime = time.time() - t_solve
    pulp_status = pulp.LpStatus.get(prob_model.status, str(prob_model.status))
    status = pulp_status.lower().replace(' ', '_')
    ...
    return {'prob': prob_val, 'path': path, 'status': status, 'runtime': runtime}
